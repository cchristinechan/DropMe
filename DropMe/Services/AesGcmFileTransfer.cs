...
public class AesGcmFileTransfer<T>(T stream, string peerName, IStorageService storageService)
    where T : Stream {
    ...
    private Task? _kaLoop;
    private CancellationTokenSource? _cts;

    public Task Start(CancellationToken ct) {
        _cts = new CancellationTokenSource();
        var linked = CancellationTokenSource.CreateLinkedTokenSource(ct, _cts.Token);
        _recvLoop = ReceiveLoop(linked.Token);
        _kaLoop = KeepAliveLoop(linked.Token);
        return Task.CompletedTask;
    }

    private async Task ReceiveLoop(CancellationToken ct) {
        while (!ct.IsCancellationRequested) {
            var (type, payload) = await SessionFraming.ReadAsync(stream, ct).ConfigureAwait(false);
    ...
    private async Task SendMessageAsync(SessionMessageType type, ReadOnlyMemory<byte> payload, CancellationToken ct) {
        ...
        try {
            await SessionFraming.WriteAsync(stream, type, payload, ct).ConfigureAwait(false);
        }
        finally {
            _writeLock.Release();
        }
    ...
    private async Task HandleFileOfferAsync(byte[] payload, CancellationToken ct) {
        ...
        var accept = FileOfferDecision is null || await FileOfferDecision(info).ConfigureAwait(false);
        ...
        if (_rxStream != null) await _rxStream.DisposeAsync();
        _rxHash?.Dispose();
        ...
        var output =
            storageService.OpenDownloadFileWriteStreamAsync(
    ...
    public async Task StopAsync() {
        ...
        try {
            if (_rxStream != null) {
                await _rxStream.DisposeAsync().ConfigureAwait(false);
            }
        }
        catch (Exception e) {
            // Logging needed?
        ...
        try {
            if (_recvLoop is not null && _kaLoop is not null)
                await Task.WhenAll(_recvLoop, _kaLoop);
        }
        catch (Exception e) {
            // Logging needed?
        }
